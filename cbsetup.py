...
def write_instruction_file(path: Path, content: str, marker_comment: str) -> bool:
    ...
    if path.exists():
        ...
        if marker_comment:
            ...
            # Markdown: wrap in markers from creation so a later cbsetup run
            # replaces the section in place instead of appending a duplicate.
            path.write_text(f"{full_section}\n")
        return True
