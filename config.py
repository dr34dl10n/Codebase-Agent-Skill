...
_MODEL_MAX_TEXT = {
    ...
    # Cap at ~2000 tokens (≈8000 chars) as a safety net. With definition
    # splitting (parser.py::_split_definition) chunks are ≤ max_chunk_size
    # (2000 chars), so this rarely triggers — but it prevents a single
    # runaway chunk from dominating embedding time on CPU (see ACCELERATION.md).
    "modernbert-embed-base":  1200,
    "modernbert-embed-large": 1200,
}
...
class ParseConfig:
    ...
    # Directories to skip (non-source). Note: any path component that is
    # hidden (starts with '.', e.g. .git, .venv, .gradle, .idea, .next) is
    # skipped GLOBALLY regardless of this list — see parser._is_dev_irrelevant.
    # So this set only needs non-hidden dir names that aren't real dev source.
    skip_dirs: set = field(default_factory=lambda: {
        # JS / TS / front-end build output
        "node_modules", "dist", "build", "out",
        # Python
        "__pycache__", "venv", "env", "_build",  # _build = Sphinx docs
        # Rust / Go / .NET / JVM / C
        "target", "obj", "Debug", "Release",
        # Vendored / vendored dependencies
        "vendor", "third_party", "third-party", "deps", "dependencies",
        "external", "submodules", "Pods", "DerivedData",
        # Coverage / autotools / CMake state
        "coverage", "build-aux", "autom4te.cache", "cmake-build-debug",
    })
    # Path segments to skip (substring match against the relative path).
    ...
    # Filenames to skip (glob, matched against the basename only). Lockfiles
    # are regenerated by tooling, minified/bundled files are build output, and
    # `*.generated.*` / `*.gen.*` mark generated code — none are useful dev
    # source. Keeps the index focused on hand-written code.
    skip_filename_patterns: list = field(default_factory=lambda: [
        # lockfiles (regenerated, not source)
        "package-lock.json", "pnpm-lock.yaml", "yarn.lock", "bun.lockb",
        "composer.lock", "Gemfile.lock", "Cargo.lock", "poetry.lock",
        "uv.lock", "go.sum", "packages.lock.json", "mix.lock", "Podfile.lock",
        # minified / bundled output
        "*.min.js", "*.min.mjs", "*.min.cjs", "*.min.css",
        # generated code / sourcemaps
        "*.generated.*", "*.gen.*", "*.map",
    ])
